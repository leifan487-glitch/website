  ...
  return <>
    <main id="main-content" ref={mainRef} className="aligned-page technology-aligned">
      ...
      <section className="aligned-hero page-shell" aria-labelledby="technology-title">
        <div><h1 id="technology-title">技术体系</h1><p className="aligned-lead">本体、遥操作、智能体、具身模型与云平台。</p><p>五项技术，连接机器人与数据工作流。</p>
          <nav className="aligned-index" aria-label="技术平台定位">{alignedPlatforms.map((p, index) => <a key={p.id} href={"#" + p.id}><span>{p.nameZh}{p.id === "bw-brain" ? <small>BW Brain</small> : null}</span>{index < alignedPlatforms.length - 1 ? <ArrowRight size={16} aria-hidden="true" /> : null}</a>)}</nav>
        </div>
      ...
      <section className="aligned-section aligned-tint" data-motion-section aria-labelledby="ecosystem-title"><div className="page-shell">
        <h2 id="ecosystem-title" data-motion-copy>开发生态</h2><p className="aligned-intro">连接仿真、控制、数据与操作验证。工具按配置与开发方案选用，非所有设备默认配备。</p>
